...
export async function POST(request: Request) {
  try {
    ...
    <div style="font-family: Arial, sans-serif; color: #333; padding: 20px; border-radius: 8px;">
      <div style="background-color: black; padding: 10px; border-radius: 8px; display: inline-block;">
        <img src="${process.env.NEXT_PUBLIC_BASEURL}/logo.png" alt="Logo" style="width: 50px;" />
      </div>
      <h2 style="color: #B38E41;">Demande de propriété</h2>
      <p>Bonjour ${formData.first_name} ${formData.last_name},</p>
      <p>Nous avons bien reçu votre demande de propriété avec les détails suivants :</p>
    ...
    const adminMailOptions = {
      from: '"NextImmo" <noreply@nextimmo.lu>',
      to: recipientEmails[formData.emailType] || process.env.ADMIN_EMAIL,
